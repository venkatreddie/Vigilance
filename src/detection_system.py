...
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Timestamp", "Event", "Yaw_deg", "Pitch_deg", "EAR", "MAR"])
...
def log_event(event, yaw=0, pitch=0, ear=0, mar=0):
    ...
    with open(LOG_FILE, "a", newline="") as f:
        csv.writer(f).writerow([timestamp, event, round(yaw,2), round(pitch,2), round(ear,2), round(mar,2)])
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            if mar > MAR_THRESHOLD:
                mar_counter +=1
                if mar_counter >= MAR_CONSEC_FRAMES and not yawn_alerted:
                    beep()
            ...
            if is_distracted:
                ...
                elif now - distraction_start >= DISTRACTION_DURATION and not distraction_alerted:
                    beep()
