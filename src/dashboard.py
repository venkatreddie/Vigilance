        ...
        with mp_face_mesh.FaceMesh(refine_landmarks=True, min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
            try:
                while st.session_state.running:
                    ...
                    ear, mouth_ratio, yaw_deg, pitch_deg = 0.0, 0.0, 0.0, 0.0
                    face_present = False
                    ...
                    if results.multi_face_landmarks:
                        ...
                        mouth_ratio = mouth_open_ratio(lm_pts)
                        try:
                    ...
                    # Yawning detection
                    if face_present and mouth_ratio > YAWN_RATIO_THRESHOLD:
...
# ---------------------- ANALYTICS SECTION ----------------------
st.markdown("---")
st.subheader("📊 Detection Analytics & History Dashboard")
...
if os.path.exists(LOG_FILE):
    try:
        ...
        # ✅ normalize Event names to prevent duplicates
        df_all["Event"] = (
            df_all["Event"].astype(str).str.strip().str.title()
            .replace({
        ...
        if not df_all.empty:
            ...
            selected_events = col1.multiselect("Select Event Types", event_types, default=event_types)
            ...
            df_filtered = df_all[df_all['Event'].isin(selected_events)]
            ...
            colB.metric("Last Event", last_event_time.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(last_event_time) else "N/A")
            colC.metric("Unique Events", ", ".join(unique_events.keys()) if unique_events else "None")
